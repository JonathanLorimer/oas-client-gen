...
constructEnvironment :: Maybe ComponentObject -> Environment
constructEnvironment mco =
  case mco of
    Nothing -> Environment M.empty M.empty M.empty M.empty
    Just co ->
      Environment
        { schemas = maybe M.empty (M.map Direct . M.mapKeys ("#/components/schemas/" <>)) co.schemas
