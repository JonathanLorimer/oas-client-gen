...
data Amazon = Amazon
  { cellularPhoneCase :: Maybe CellularPhoneCase
  , decorativeCutout :: Maybe DecorativeCutout
  , displayAlbum :: Maybe DisplayAlbum
  , drinkingCup :: Maybe DrinkingCup
  , marketplace :: Text
  , officeProducts :: Maybe OfficeProducts
  , shirt :: Maybe Shirt
  , sku :: Text
  , sweatshirt :: Maybe Sweatshirt
  , toteBag :: Maybe ToteBag
  , wallArt :: Maybe WallArt
  }
  deriving (Show, Eq)

instance FromJSON Amazon where
  parseJSON = withObject "Amazon" $ \obj -> do
    cellularPhoneCase <- obj .: "cellular_phone_case"
    decorativeCutout <- obj .: "decorative_cutout"
    displayAlbum <- obj .: "display_album"
    drinkingCup <- obj .: "drinking_cup"
    marketplace <- obj .: "marketplace"
    officeProducts <- obj .: "office_products"
    shirt <- obj .: "shirt"
    sku <- obj .: "sku"
    sweatshirt <- obj .: "sweatshirt"
    toteBag <- obj .: "tote_bag"
    wallArt <- obj .: "wall_art"
    pure $ Amazon{..}

instance ToJSON Amazon where
  toJSON Amazon{..} =
    object [
      "cellular_phone_case" .= cellularPhoneCase,
      "decorative_cutout" .= decorativeCutout,
      "display_album" .= displayAlbum,
      "drinking_cup" .= drinkingCup,
