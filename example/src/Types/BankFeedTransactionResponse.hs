...
module Types.BankFeedTransactionResponse where
...
data BankFeedTransactionResponse = BankFeedTransactionResponse
  { amount :: Maybe Double
...
instance FromJSON BankFeedTransactionResponse where
  parseJSON = withObject "BankFeedTransactionResponse" $ \obj -> do
    amount <- obj .: "amount"
    ...
    pure $ BankFeedTransactionResponse{..}

instance ToJSON BankFeedTransactionResponse where
  toJSON BankFeedTransactionResponse{..} =
    object [
      "amount" .= amount,
